...
namespace LatihanADONET
{
    public partial class Form1 : Form
    {
        ...
        private SQLiteConnection GetOpenConnection()
        {
            ...
            try // penggunaan blok try-catch untuk penanganan error
            {
                ...
                string dbName = @"D:\informatika\if semester 3\pemrograman lanjut\Module Praktikum 08 (ADO.NET)\LatihanADO.NET\Database\DbPerpustakaan.db";
                ...
                
 {
        ...
        {
            // Validasi bahwa NPM disediakan untuk penghapusan
            if (string.IsNullOrEmpty(txtNpmDelete.Text))
            {
                MessageBox.Show("NPM harus diisi untuk menghapus data !!!", "Informasi", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                txtNpmDelete.Focus();
                return;
            }

            // Minta konfirmasi sebelum penghapusan
            DialogResult result = MessageBox.Show("Apakah Anda yakin ingin menghapus data mahasiswa?", "Konfirmasi", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result == DialogResult.Yes)
            {
                // Melakukan penghapusan
                DeleteMahasiswa(txtNpmDelete.Text);
            }
        }
        ...
        private void DeleteMahasiswa(string npm)
        {
            ...
            if (result > 0)
            {
                ...
                // You can also refresh the displayed data if needed
                btnTampilkanData_Click(null, null);
            }
            else
            {
        ...
        private void btnCariDelete_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(txtNpmDelete.Text))
            {
                MessageBox.Show("NPM harus diisi untuk mencari data !!!", "Informasi", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                txtNpmDelete.Focus();
                return;
            }

            // Membuat objek Koneksi, sekaligus membuka koneksi ke database
            SQLiteConnection conn = GetOpenConnection();
